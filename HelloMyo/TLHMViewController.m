...
- (void)didReceivePoseChange:(NSNotification *)notification {
    ...
    switch (pose.type) {
        ...
        case TLMPoseTypeWaveIn:
            ...
            self.helloLabel.textColor = [UIColor greenColor];
            [self play:((TLMArmRight == pose.myo.arm) ? @"right"  : @"left")];
            break;
        case TLMPoseTypeWaveOut:
            ...
            self.helloLabel.textColor = [UIColor greenColor];
            [self play:((TLMArmRight == pose.myo.arm) ? @"left" : @"right")];
            break;
...
- (void)play:(NSString *)file {
  SystemSoundID soundID = (SystemSoundID) [[_soundDict objectForKey:file] integerValue];
  if (!soundID) {
    NSURL *soundURL = [[NSBundle mainBundle] URLForResource:file withExtension:@"caf"];
    if (soundURL) {
      AudioServicesCreateSystemSoundID((__bridge CFURLRef)soundURL, &soundID);
      if (soundID) {
        [_soundDict setObject:@(soundID) forKey:file];
      }
    }
  }
  if (soundID) {
